...
save('all_corsi_log_v1', 'all_corsi')
...
for jid_type = 1:4
    ...
    regressor_vals = regressor_vals(L, [1, 3]);
    [res.val.masks, res.val.p_vals, res.val.mdl, res.val.A, res.val.B] = singleDayLIMO(regressor_vals, with_jid.jids(:, jid_type));
...
save('all_2back_log_v1', 'all_2back')
...
for idx_val = 1:2
    for jid_type = 1:4
        ...
        fprintf("\tTest value + gender\n");
        regressor_vals = table2array(with_jid(:, {'vals', 'gender'}));
        regressor_vals = regressor_vals(L, [idx_val, 3]);
        [res.val.masks, res.val.p_vals, res.val.mdl, res.val.A, res.val.B] = singleDayLIMO(regressor_vals, with_jid.jids(:, jid_type));
...
save('all_rtime_log_v1', 'all_r_time')
...
save('all_switch_log_v1', 'all_switch')
...
save('all_multitest_log_v1', 'all_multitest')
